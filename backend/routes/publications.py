...
def delete_publication(pub_id):
    ...
    db = SessionLocal()
    try:
        p = db.query(Publication).filter(Publication.id == pub_id).first()
        if not p: return jsonify({"error": "No encontrada"}), 404
        if p.user_id != user_id: return jsonify({"error": "No es tu receta"}), 403
...
def edit_publication(pub_id):
    ...
    try:
        ...
        description = request.form.get('description', p.body)
        try:
        ...
        except: return jsonify({"error": "Formato incorrecto"}), 400
        meta = p.image_meta if isinstance(p.image_meta, dict) else {}
        if isinstance(p.image_meta, str):
            try: meta = _json.loads(p.image_meta)
            except: meta = {}
        new_urls = list(meta.get('urls', []))
        for i in range(3):
            file = request.files.get(f'file_{i}')
            if file and file.filename:
                url = upload_to_cloudinary(file)
                if i < len(new_urls): new_urls[i] = url
                else: new_urls.append(url)
        p.title = title; p.body = description
        p.image_meta = {"urls": new_urls, "ingredients": list(ingredients),
